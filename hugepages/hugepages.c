...
int main(int argc, char *argv[]) {
    printf("Huge pages example program\n");
    ...
    // mmap() flags
    // MAP_PRIVATE  : process private memory, not shared memory
    // MAP_HUGETLB  : use huge pages
    // MAP_ANONYMOUS: the mapping is not backed by any file (i.e. just allocate memory, don't map file into it)
    int FLAGS = MAP_PRIVATE | MAP_HUGETLB | MAP_ANONYMOUS;
    ...
    buffer = mmap(NULL                     /* NULL = let kernel decide start address of mapped memory */, 
                 ...
                 FLAGS,
                 -1,                       /* fd can be ignored since we're using MAP_ANONYMOUS */
                 0                         /* read offset */);
